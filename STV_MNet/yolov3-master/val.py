...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(preds):
            ...
            if plots:
                img=im[si:si + 1]
                plot_images(im[si:si + 1], output_to_target(pred[None]), [path],
                            save_dir / f'{path.stem}.jpg', names)#output_to_target(pred[None])
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/Tree.yaml', help='dataset.yaml path')
